...
<html>
 <head>
  ...
  <title>图片信息列表</title>
  <style>
  body{background: url('../uploads/body_bg.jpg' ) no-repeat; background-size:120%;}
 ...
 <body>
	<div class="box">
	<div class="title">图 片 列 表</div>
 	<div class="search">
		<form method="get" action="tpindex.php">
	...
	<table border="1">
  	...
  	<?php
        ...
        if(!empty($_GET['keywords'])){//判断搜索条件是否存在
          $where = " where word like '%".$_GET['keywords']."%'";
      ...
      ?>
		<tr>
			 ...
			 <td><img src="../<?php echo $row['url']; ?>" width="65" alt="图片加载失败"></td>
			 <td><?php echo $row['fbl']; ?></td>
             <td><?php echo $row['dwl']; ?></td>
            ...
            <?php echo $status[$row['status']];?></a></td>
			 ...
			 <td>
				<div align="center">
					<span>
						<a class='page' href="edits.php?id=<?php echo $row['id'];?>">编辑</a> | 
						<a class='page' href="javascript:delSure(<?php echo $row['id'].",'".$row['url']."'"; ?>)">删除</a>
					</span>
				</div>
			</td>
		</tr>
		...
		<tr> 
			<td  colspan="8" class="fenye"><?php echo $maxrow; ?> 条数据 <?php echo $page."/"; echo $maxpage; ?> 页&nbsp;&nbsp;
				<?php
				echo "<a class='page' href='tpindex.php?p=1{$url}'>首页</a>&nbsp;&nbsp;";
				echo "<a class='page' href='tpindex.php?p=".($page-1)."{$url}'>上一页</a>&nbsp;&nbsp;";
				echo "<a class='page' href='tpindex.php?p=".($page+1)."{$url}'>下一页</a>&nbsp;&nbsp;";
				echo "<a class='page' href='tpindex.php?p={$maxpage}{$url}'>尾页</a>";
				?>
			</td>
		</tr>
	</table>
...
function delSure(id,img){
  if(confirm("您确定要删除吗?")){
      $.post("actions.php",{a:"del",id:id,img:img},function(data){
        if (data == 0) {
            alert("删除成功！");
            window.location.href= "tpindex.php";
        }else { 
            alert("删除失败！");
        };
      },"json");  
...
function doChange(id, state){//改成物品的展示状态
    ...
    if(confirm(msg)){
        $.post("actions.php",{a:"change",id:id,status:state},function(data){
        if (data==0) { 
            alert("修改成功！");
            window.location.href= "tpindex.php";
        }else { 
